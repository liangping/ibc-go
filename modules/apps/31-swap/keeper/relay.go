...
func (k Keeper) SendSwap(
	...
	// 	amount, _ := sdk.NewIntFromString(sendToken.Amount)
	// 	if err := k.bankKeeper.SendCoins(
	// 		ctx, sender, escrowAddress, sdk.NewCoins(sdk.NewCoin(sendToken.Denom, amount)),
	// 	); err != nil {
	// 		return err
	// 	}
	...
	packetData := types.NewFungibleTokenSwapPacketData(
		supplyToken, demandToken, sender.String(), receiver, counterPartyAddress,
	)
	...
	defer func() {
		...
		telemetry.IncrCounterWithLabels(
			[]string{"ibc", types.ModuleName, "swap"},
			1,
			labels,
		)
	}()
...
func (k Keeper) OnRecvPacket(ctx sdk.Context, packet channeltypes.Packet, data types.FungibleTokenSwapPacketData) error {
	...
	receiver, err := sdk.AccAddressFromBech32(data.AddressToReceiveDemandToken)
	if err != nil {
		return err
	}

	ctx.Logger().Debug("Swap module receive packets:", receiver, data.SupplyToken, data.DemandToken, data.AddressToReceiveDemandToken, data.CounterPartyAddress)
...
func (k Keeper) OnAcknowledgementPacket(ctx sdk.Context, packet channeltypes.Packet, data types.FungibleTokenSwapPacketData, ack channeltypes.Acknowledgement) error {
...
func (k Keeper) OnTimeoutPacket(ctx sdk.Context, packet channeltypes.Packet, data types.FungibleTokenSwapPacketData) error {
	return k.refundPacketToken(ctx, packet, data)
}
...
// the sending address.
func (k Keeper) refundPacketToken(ctx sdk.Context, packet channeltypes.Packet, data types.FungibleTokenSwapPacketData) error {
	// NOTE: packet data type already checked in handler.go

	token, err := sdk.ParseCoinNormalized(data.SupplyToken)
	if err != nil {
		return err
	}
	// decode the sender address
	sender, err := sdk.AccAddressFromBech32(data.SendAddress)
	if err != nil {
		return err
	}

	println(token, sender)
