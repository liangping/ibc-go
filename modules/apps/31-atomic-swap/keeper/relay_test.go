	...
	for _, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			if tc.recvIsSource {
				...
				sendingcoin := sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(100))
				receivingcoin := sdk.NewCoin("uosmo", sdk.NewInt(500))
				...
				res, err := suite.chainB.SendMsgs(swapMsg)
				suite.Require().NoError(err) // message committed
			...
			transferMsg := types.NewMsgSwap(path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID,
				sdk.NewCoin(sdk.DefaultBondDenom, amount),
				sdk.NewCoin("uosmo", sdk.NewInt(500)),
				suite.chainA.SenderAccount.GetAddress().String(),
				receiver, "", clienttypes.NewHeight(0, 110), 0)
			_, err := suite.chainA.SendMsgs(transferMsg)
			suite.Require().NoError(err) // message committed

			tc.malleate()

			//data := types.NewAtomicSwapPacketData(
			//	trace.GetFullDenomPath(), amount.String(),
			//	suite.chainA.SenderAccount.GetAddress().String(),
			//	receiver)
			...
			//
			//err = suite.chainB.GetSimApp().SwapKeeper.OnRecvPacket(suite.chainB.GetContext(), packet, data)
